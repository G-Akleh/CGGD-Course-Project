...
{
	//Lab 3.01. Add `model` and `camera` creation code into `init` method of `dx12_renderer` class
...
{
	//Lab 3.06. Implement `render` method
...
{
	//Lab 3.02. Enable a validation layer
...
#endif
	//Lab 3.02. Create a DXGI factory
...
{
	//Lab 3.02. Enumerate hardware adapters
...
{
	//Lab 3.02. Create a command queue
...
{
	//Lab 3.02. Create a swap chain and bind it to window
...
{
	//Lab 3.04. Create a descriptor heap for render targets
	rtv_heap.create_heap(
			device,
			D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
			frame_number);
	//Lab 3.04. Create render target views
...
{
	//Lab 3.06. Create command allocators and a command list
...
{
	//Lab 3.02. Bring everything together in `load_pipeline` method
...
{
	//Lab 3.05. Create a descriptor table and a root signature
...
{
	//Lab 3.05. Setup a PSO descriptor and create a PSO
	// Compile shaders
...
{
	//Lab 3.03. Implement resource creation on upload heap
...
{
	//Lab 3.03. Map resources and copy suitable data to the resources
...
{
	//Lab 3.04. Create vertex buffer views
...
{
	//Lab 3.04. Create index buffer views
...
{
	//Lab 3.06. Implement `populate_command_list` method
	// Reset
	...
	command_list->OMSetRenderTargets(
			...
			FALSE,
			nullptr);
	const float clear_color[] = {0.f, 0.f,
								 0.f, 1.f};
	...
	for (size_t s = 0; s < model->get_vertex_buffers().size(); ++s) {
		...
		command_list->DrawIndexedInstanced(
				static_cast<UINT>(model->get_index_buffers()[s]
...
{
	//Lab 3.07. Implement `move_to_next_frame` method
	...
	if (fence->GetCompletedValue() < fence_values[frame_index])
	{
		THROW_IF_FAILED(fence->SetEventOnCompletion(
				...
				fence_event));
		WaitForSingleObjectEx(fence_event,
							  INFINITE, FALSE);
	}
...
{
	//Lab 3.04. Implement `create_heap`, `get_cpu_descriptor_handle`, `get_gpu_descriptor_handle`, and `get` methods of `cg::renderer::descriptor_heap`
