...
{
	//Lab 3.08. Implement `get_dxm_view_matrix`, `get_dxm_projection_matrix`, and `get_dxm_mvp_matrix` methods of `camera`
	DirectX::FXMVECTOR eye_position{position.x,
									position.y,
									position.z};
	DirectX::FXMVECTOR eye_direction{get_direction().x,
									 get_direction().y,
									 get_direction().z};
	DirectX::FXMVECTOR up_direction{get_up().x,
									get_up().y,
									get_up().z};
